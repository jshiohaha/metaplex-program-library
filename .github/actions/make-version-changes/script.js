...
const updateCratesPackage = async (io, cwdArgs, pkg, semvar) => {
  ...
  // if we globally installed `@iarna/toml`, the root `yarn.lock` file will have been committed along with `cargo release` command. so, we need to resolve this.
  wrappedExec(`git restore --source=HEAD^ --staged -- ${[...rootDir, 'yarn.lock'].join('/')} && git commit --amend --allow-empty -C HEAD`);
